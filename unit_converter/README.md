...
| `ConverterProvider.result` is a computed getter | No separate state field to keep in sync; recomputes from `_inputText`, `_fromUnit`, and `_toUnit` on every rebuild |
...
│   │   └── app_strings.dart                   # All UI labels and hint strings
│   ├── data/
│   │   └── model/
│   │       └── unit_category_model.dart       # Static categories + full conversion engine
│   ├── domain/
│   │   └── entities/
│   │       └── unit_category.dart             # Pure UnitCategory entity
│   └── presentation/
│       ├── provider/
│       │   └── converter_provider.dart        # Selected category, units, input, result state
│       └── screen/
│           ├── converter_screen.dart          # Main conversion screen
│           └── widget/
│               ├── category_selector.dart     # Horizontal chip selector
│               ├── input_field.dart           # Numeric text input
│               ├── unit_dropdown.dart         # From / To unit dropdown
│               └── result_card.dart           # Converted result display
├── android/
├── ios/
├── test/
├── pubspec.yaml
...
1. **Select a category** — Tap a chip (Length, Weight, or Temperature) to switch the active unit set. The From/To dropdowns and the input field reset.
...
4. **Swap** — Tap the swap icon between the dropdowns to reverse From and To in one tap.
